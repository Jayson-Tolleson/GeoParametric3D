 ...
 *  - 100% Opaque Solid Shading (FreeCAD parity): solids default to full opacity with hardware depth buffer occlusion & directional diffuse lighting.
 *  - Multi-solid color assignment preserved directly from STEP header metadata.
 *  - Bidirectional property panel reaction (Material, Color, Opacity, Position, Rotation, Scale).
...
function shadeColor(colorStr, intensity = 1.0, opacity = 1.0, isSelected = false) {
  let { r, g, b } = parseColorToRgb(colorStr);
  if (isSelected) {
    r = Math.min(255, Math.round(r * 0.5 + 251 * 0.5));
    g = Math.min(255, Math.round(g * 0.5 + 191 * 0.5));
    b = Math.min(255, Math.round(b * 0.5 + 36 * 0.5));
  } else {
  ...
  const a = Math.max(0.05, Math.min(1.0, opacity));
  return `rgba(${r}, ${g}, ${b}, ${a})`;
}
...
export class ViewportController {
  ...
  syncNativePolygons(map3dElement, objects) {
    ...
    objects.forEach(object => {
      ...
      const baseOpacity = typeof object.opacity === 'number' ? object.opacity : 1.0;
      ...
      if (planarPolys.length > 0) {
        planarPolys.forEach((poly, polyIndex) => {
          ...
          const fillColor = isFaceSel ? 'rgba(251, 191, 36, 1.0)' : shadeColor(baseColor, 1.0, baseOpacity, isObjSel && selMode === 'part');
          const strokeColor = isFaceSel ? '#ffffff' : (isObjSel ? '#ffffff' : 'rgba(0,0,0,0.4)');
  ...
  render() {
    ...
    // 1. Draw Ground Grid & Coordinate Axes
    ...
    objects.forEach(obj => {
      ...
      const scale = obj.scale || [1, 1, 1];
      const faces = obj.faces || [];
      const partColor = obj.color || '#38bdf8';
      const partOpacity = typeof obj.opacity === 'number' ? obj.opacity : 1.0;
      ...
      faces.forEach((face, fIdx) => {
        ...
        // Compute face normal for diffuse shading
        let faceNorm = [0, 0, 1];
    ...
    // 3. Render Solid Opaque Shaded Faces with Directional Diffuse Lighting
    queue.forEach(item => {
      const poly = item.poly2D;
      if (poly.length < 3) return;

      const dot = item.normal[0] * lightDir[0] + item.normal[1] * lightDir[1] + item.normal[2] * lightDir[2];
      const intensity = Math.max(0.4, Math.min(1.0, 0.65 + 0.35 * Math.abs(dot)));
      ...
      if (item.isFaceSel) {
        ...
        ctx.fillStyle = shadeColor(item.faceColor, intensity, item.opacity, item.isObjSel && selMode === 'part');
      }
      ctx.fill();

      ctx.lineWidth = item.isFaceSel ? 2 : (item.isObjSel ? 1.5 : 0.75);
      ctx.strokeStyle = item.isFaceSel ? '#ffffff' : (item.isObjSel ? 'rgba(255, 255, 255, 0.9)' : 'rgba(0, 0, 0, 0.35)');
      ctx.stroke();
    });

    // 4. Render Sub-element and Selection Highlighting
    if (selMode === 'edge') {
      ctx.lineWidth = 3;
      edges.forEach(e => {
    ...
    // 6. Render Active Draft Feedback
    ...
    // 7. Render Box Selection Marquee
