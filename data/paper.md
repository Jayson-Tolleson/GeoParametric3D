...
This specification formally addresses the architectural mandate to eliminate legacy intermediate mesh layers, details the mathematical mechanics of the Dual-Route B-Rep rendering pipeline, establishes 100% opaque solid shading parity, incorporates a 2,000-foot ground grid with a 1-foot mesh constant, and provides an exhaustive analysis of geodetic anchoring constraints, establishing Null Island (`[0.0, 0.0, 0.0]`) as the exclusive planetary geodetic origin anchor.
...
- **`<gmp-polygon-3d>`:** Direct rendering of exact planar polygons (`outerCoordinates`, `innerCoordinates`) with GPU-driven rasterization, zero meshing diagonals, lighter subtle boundary stroke lines, and hardware depth testing against 3D buildings and terrain.
